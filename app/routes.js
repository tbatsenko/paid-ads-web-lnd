...
module.exports = function factory(app, lightning, db, config) {
    ...
    postHook: (req, response) => {
      ...
      for (i = 0; i < edges.length; i += 1) {
        ...
        if (channeledNodes[edge.node1_pub] && channeledNodes[edge.node2_pub]) { // skip buggy edges
          ...
          g.addEdge(edge.node1_pub, edge.node2_pub, {
            label: edgeLabel,
            fontsize: '12.0'
          });
        }
  ...
  app.get('/api/lnd/networkgraph.svg', (req, res) => {
    ...
    return res.sendFile('networkgraph.svg', {
      root: `${__dirname}/../data/`
    });
  });
  ...
  // application -------------------------------------------------------------
  app.get('*', (req, res) => {
    res.sendFile('lnd.html', {
      root: `${__dirname}/../public/`
    }); // load the single view file (angular will handle the page changes on the front-end)
  });

  app.post('/api/lnd/openchannel', lightningRPCAdapter('openchannel', {
    isLimitedToAuthorizedUser: true,
    preHook: (req) => {
      try {
        const openChannelRequest = {
          node_pubkey: BufferUtil.hexToBuffer(req.body.pubkey),
          local_funding_amount: Number(req.body.localamt),
          push_sat: Number(req.body.pushamt),
        };

        console.log('REQUEST DATAAAAA');

        console.log(openChannelRequest.local_funding_amount);
        console.log(openChannelRequest.push_sat);

        const call = lightning.getActiveClient().openChannel(openChannelRequest);
        call.on('data', (ocdata) => {
          console.log('OpenChannel Data', ocdata);
        });
        call.on('end', () => {
          console.log('OpenChannel End');
        });
        call.on('error', (err) => {
          console.error('OpenChannel Error', err.message);
          const error = { ...err,
            error: err.message
          };
        });
